...
static void set_deposited_token_ui(ethQueryContractUI_t *msg, context_t *context)
{
    ...
    case CREATE:
        if (context->booleans & IS_COPY)
            strlcpy(msg->title, COPY_DEPOSITED_TOKEN_TITLE, msg->titleLength);
        else
            strlcpy(msg->title, CREATE_DEPOSITED_TOKEN_TITLE, msg->titleLength);
...
static void set_received_token_ui(ethQueryContractUI_t *msg, context_t *context)
{
    ...
    case CREATE:
        if (context->booleans)
        {
            strlcpy(msg->title, COPY_RECEIVED_TOKEN_TITLE, msg->titleLength);
            strlcpy(msg->msg, COPY_RECEIVED_TOKEN_MSG, msg->msgLength);
        }
        else
        {
            strlcpy(msg->title, CREATE_RECEIVED_TOKEN_TITLE, msg->titleLength);
            strlcpy(msg->msg, CREATE_RECEIVED_TOKEN_MSG, msg->msgLength);
        }
        break;
    default:
        break;
