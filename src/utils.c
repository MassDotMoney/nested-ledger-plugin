...
static uint8_t does_number_fit(const uint8_t *parameter, uint8_t parameter_length, uint8_t size) {
...
uint8_t copy_number_uint32(uint32_t *target, const uint8_t *parameter, uint8_t parameter_length) {
    if (does_number_fit(parameter, parameter_length, sizeof(*target))) return 1;
