...
void parse_order(ethPluginProvideParameter_t *msg, context_t *context) {
  ...
  case ORDER__LEN_CALLDATA:
    ...
    if (msg->parameterOffset > context->target_offset) {
      ...
      if (copy_number(&context->last_calldata_offset, msg->parameter,
                      PARAMETER_LENGTH)) {
        msg->result = ETH_PLUGIN_RESULT_ERROR;
        return;
      }
...
void parse_batched_output_orders(ethPluginProvideParameter_t *msg,
  ...
  case BOO__AMOUNT:
    ...
    if (context->current_length == 1) {
      copy_parameter(context->token1_amount, msg->parameter,
  ...
  case BOO__OFFSET_ARRAY_ORDERS:
    ...
    if (context->current_length == 0) {
      ...
      if (copy_number(&context->target_offset, msg->parameter,
                      PARAMETER_LENGTH)) {
        msg->result = ETH_PLUGIN_RESULT_ERROR;
        return;
      }
