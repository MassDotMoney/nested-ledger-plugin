...
function serialize_data(pluginName, contractAddress, selector) {
  ...
  const methodid = Buffer.from(selector.slice(2), "hex"); // Taking .slice(2) to remove the "0x" prefix
...
function generate_plugin_config() {
  ...
  var files = fs.readdirSync(`${pluginFolder}/abis/`); // `contracts_to_abis` holds a maping of contract addresses to abis

  let contracts_to_abis = {};

  for (let abiFileName of files) {
    assert(abiFileName.toLocaleLowerCase() == abiFileName, `FAILED: File ${abiFileName} should be lower case.`); // Strip ".json" suffix

    let contractAddress = abiFileName.slice(0, abiFileName.length - ".json".length); // Load abi

    let abi = require(`../${pluginFolder}/abis/${abiFileName}`); // Add it to contracts
  ...
  const PLACE_HOLDER_SIGNATURE = "3045022100f6e1a922c745e244fa3ed9a865491672808ef93f492ee0410861d748c5de201f0220160d6522499f3a84fa3e744b3b81e49e129e997b28495e58671a1169b16fa777"; // Iterate through contracts in b2c.json file
  ...
  for (let contract of b2c["contracts"]) {
    ...
    for (let [selector, values] of Object.entries(contract["selectors"])) {
      assert(selector.toLowerCase() == selector, `FAILED: Selector ${selector} should be lower case`); // Gather up the info needed for the end result
      ...
      assert(erc20OfInterest.length <= 2, `Maximum of 2 erc20OfInterest allowed. Got ${erc20OfInterest.length}`); // Put them in `methods_info`
    ...
    methods_info["abi"] = contracts_to_abis[contractAddress]; // Add the methods_info to the end result
